...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
        ...
        //
        System.out.println("incoming : link="+permalink+" ord="+ordinal);

        Document post = postsCollection.find(eq("permalink", permalink)).first();
        Document updateQuery = new Document("$inc", new Document("comments." + ordinal + ".num_likes", 1));
        postsCollection.updateOne(post,updateQuery);

//        postsCollection.update(new Document("permalink", permalink), post);
        //postsCollection.updateOne(eq("permalink",permalink),post);
    }
}
